...
| `components/` | Pre-redesign React components, frozen 2026-09-24 when the Signal & Silicon redesign stopped using them: the physics canvas backgrounds (`FluidPlasmaBackground`, `ParticleField`, `FallingChipsBackground`, `WaveInterferenceBackground`, `GradientMesh`), `MagneticWrapper`, `ResearchVisual`, `PolarPlot`, the 2D `PhasedArrayCalculator` and the form-only `FMCWRadarCalculator`. Excluded from the build, lint and type-check. | `src/components/rf/PhasedArrayLab.tsx` and `src/components/rf/fmcw/FmcwScope.tsx` for the two calculators; `docs/DESIGN.md` for the current visual system |
