...
interface PatternCutPlotProps {
  cut: Cut;
  peakDeg: number;
  /** Azimuth of the cut plane, degrees. */
  phiDeg: number;
}
...
export default function PatternCutPlot({ cut, peakDeg, phiDeg }: PatternCutPlotProps) {
  let d = '';
  ...
  return (
    <figure className="flex flex-col gap-3">
      {/* Symbols and units keep their case inside the uppercase caption. */}
      <figcaption className="kicker flex justify-between text-[10.5px]">
        <span>
          Pattern cut · <span className="normal-case">φ = {phiDeg}°</span> plane
        </span>
        <span className="normal-case">dB vs θ</span>
      </figcaption>
      <div className="grid grid-cols-[34px_1fr] gap-2">
        ...
        <svg viewBox={`0 0 ${W} ${H}`} preserveAspectRatio="none" className="h-[150px] w-full overflow-visible sm:h-[180px]" role="img" aria-label={`Pattern cut in the phi ${phiDeg} degree plane. Beam peak at ${peakDeg.toFixed(1)} degrees.`}>
