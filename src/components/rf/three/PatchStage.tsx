...
/**
 * Two-slot (cavity) pattern of a rectangular patch: resonant length L along the
 * three.js z-axis, width W along x. Upper hemisphere only.
 */
function buildLobe(W: number, L: number, freqGHz: number, scale: number, baseY: number, p: StagePalette) {
  ...
  for (let i = 0; i < nT; i++) {
    ...
    for (let j = 0; j < nP; j++) {
      ...
      const m = Math.abs(sinc * Math.cos((k0 * L * ux) / 2) * Math.cos(th));
      mags.push(m);
      if (m > peak) peak = m;
    }
  }
...
export default function PatchStage({ widthMm, lengthMm, heightMm, freqGHz, label }: PatchStageProps) {
  ...
  useEffect(() => {
    if (!runtime || ![widthMm, lengthMm, heightMm, freqGHz].every((v) => Number.isFinite(v) && v > 0)) return;
    const p = readPalette();
    ...
    const lobe = buildLobe(widthMm, lengthMm, freqGHz, 1.2, h + 0.02, p);
    root.add(lobe);
  ...
  }, [runtime, widthMm, lengthMm, heightMm, freqGHz, themeTick]);
  ...
  return (
    ...
    >
      ...
      <p className="pointer-events-none absolute inset-x-0 bottom-0 bg-gradient-to-t from-bg-raised via-bg-raised/85 to-transparent px-4 pb-3 pt-10 text-[11.5px] leading-snug text-ink-3">
        Ground plane drawn at twice the larger patch side. Fringing fields at the two radiating edges swing in phase; the translucent lobe is the broadside pattern.
      </p>
    </ThreeCanvas>
  );
}
