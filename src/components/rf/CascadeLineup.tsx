...
function LevelDiagram({ stages, plan, bandwidthLabel }: { stages: LineupStage[]; plan: LevelPlan; bandwidthLabel: string }) {
  ...
  return (
    // A dark scrollbar in dark mode; the site does not set color-scheme globally.
    <div ref={ref} className="overflow-x-auto dark:[color-scheme:dark]">
      <svg
        width={width}
        height={H}
        viewBox={`0 0 ${width} ${H}`}
  ...
  return (
    <div className="bg-surface p-5">
      <div className="flex flex-wrap items-baseline justify-between gap-x-4 gap-y-1">
        <h4 className="kicker text-[11px]">{title}</h4>
        <span className="text-[12px] text-ink-3">{note}</span>
      </div>
      <div className="mt-4 flex h-2.5 w-full gap-px overflow-hidden rounded-full bg-surface-2" aria-hidden="true">
        {stages.map((s, i) => (
      ...
      <ol className="mt-4 flex flex-col gap-2">
        {stages.map((s, i) => (
          <li key={s.id} className="flex items-center gap-3 text-[13px]">
            <span className="size-2.5 shrink-0 rounded-full" style={{ background: stageColor(i) }} aria-hidden="true" />
...
export default function CascadeLineup({ stages, frequencyLabel }: { stages: LineupStage[]; frequencyLabel?: string }) {
  ...
  return (
    // Controls sit above the diagram so the level plot gets the full width of the content column.
    <div className="@container flex flex-col gap-6">
      <div className="overflow-hidden rounded-xl border border-line">
        <div className="grid gap-6 border-b border-line p-5 @2xl:grid-cols-2 @2xl:gap-x-8">
          <div className="flex flex-col gap-6">
            <div>
              <h4 className="kicker text-[11px]">Level diagram</h4>
          ...
          <div className="grid grid-cols-1 gap-5 border-t border-line pt-5 @md:grid-cols-3 @2xl:grid-cols-1 @2xl:border-l @2xl:border-t-0 @2xl:pl-8 @2xl:pt-0">
            <Readout label="Output SNR" value={level(plan.snrOutDB)} unit="dB" tone={plan.snrOutDB < 0 ? 'alert' : 'accent'} />
        ...
        <figure className="min-w-0 bg-bg-raised">
          <div className="flex flex-wrap items-center gap-x-5 gap-y-2 border-b border-line px-5 py-3 text-[11.5px] font-medium text-ink-3">
          ...
          <figcaption className="border-t border-line px-5 py-3 text-xs leading-relaxed text-ink-3">
            <span className="mb-1 block text-[11.5px] font-medium text-ink-2 @2xl:hidden">Swipe the diagram sideways to see every stage.</span>
            Matched-stage Friis lineup{frequencyLabel ? ` ${frequencyLabel}` : ''}: the noise floor is kT₀B·F·G with kT₀ = −174 dBm/Hz (290 K), cascaded IP3 adds 1/IIP3 terms in phase (the worst case), and SFDR = ⅔(IIP3 − kT₀B − NF). Signal levels are small-signal, with no compression modeled. For a memoryless cubic nonlinearity the input P1dB sits 9.6 dB below IIP3, so a stage with under about 10 dB of OIP3 headroom is already near compression.
          </figcaption>
        </figure>
      </div>

      <div className="grid gap-px overflow-hidden rounded-xl border border-line bg-line @xl:grid-cols-2">
