...
export default function FmcwScope() {
  ...
  return (
    // Controls sit above the scope so the plots get the full width of the content column.
    <div className="@container flex flex-col gap-6">
      <div className="overflow-hidden rounded-xl border border-line">
        <div className="grid gap-x-8 gap-y-6 border-b border-line p-5 @xl:grid-cols-2">
      ...
      {/* The last cell spans two columns until all five fit on one row, so the hairline grid never leaves a gap. */}
      <div className="grid grid-cols-2 gap-px overflow-hidden rounded-xl border border-line bg-line @xl:grid-cols-3 @3xl:grid-cols-5 [&>*:last-child]:col-span-2 @3xl:[&>*:last-child]:col-span-1">
      ...
      <p className="text-xs leading-relaxed text-ink-3">
        Ideal linear chirp and stationary point targets: ΔR = c / 2B, S = B / Tc, τ = 2R / c, f_b = S·τ, and R_max = f_IF·c / 2S, the beat-frequency limit alone. The range profile is the windowed-FFT response of the beat tone (a sinc for the rectangular window; its Hann counterpart has lower sidelobes and a wider main lobe). The IF is complex (IQ), so the beat spectrum has no mirror image. Two targets are equal in strength and add in power, the average over their random relative phase. Chirps and delays are drawn to scale and the magnifier states its zoom; the sweep cursor and the scrolling IF trace are slowed-down illustrations. Doppler, phase noise, chirp nonlinearity and ADC sampling are not modeled.
      </p>
    </div>
  );
}
