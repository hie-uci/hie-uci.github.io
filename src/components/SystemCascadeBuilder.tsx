...
function NodeNumber({ label, unit, value, onCommit, disabled, title }: { label: string; unit: string; value: number; onCommit: (v: number) => void; disabled?: boolean; title?: string }) {
  ...
  return (
    <div className="flex items-center justify-between gap-3">
      <label htmlFor={id} className="font-mono text-[10.5px] uppercase tracking-[0.1em] text-ink-3 [font-stretch:87.5%]">
      ...
      <input
        ...
        className="nodrag h-8 w-[76px] rounded-[3px] border border-line-strong bg-bg-raised px-2 text-right font-mono text-[12px] tabular-nums text-ink [appearance:textfield] [font-stretch:87.5%] hover:border-ink-3 focus:border-accent-ink focus:outline-none focus:ring-2 focus:ring-accent-ink/25 disabled:cursor-not-allowed disabled:opacity-50 [&::-webkit-inner-spin-button]:appearance-none [&::-webkit-outer-spin-button]:appearance-none"
      />
    </div>
  );
}
    ...
    <div className="border-t border-line px-3 py-2.5">
      {data.sParamData ? (
        <div className="flex items-center gap-2">
          <span className="min-w-0 flex-1 truncate font-mono text-[10.5px] text-trace [font-stretch:87.5%]" title={data.sParamFileName}>
            S21 · {data.sParamFileName}
          </span>
          <button type="button" className="nodrag font-mono text-[10.5px] text-ink-3 underline-offset-2 hover:text-ink hover:underline" onClick={() => cascade.clearFile(id)}>
            Unload
          </button>
        </div>
      ) : (
        <label className="nodrag flex cursor-pointer items-center gap-2 rounded-[3px] font-mono text-[10.5px] text-ink-2 [font-stretch:87.5%] hover:text-ink has-[:focus-visible]:outline has-[:focus-visible]:outline-2 has-[:focus-visible]:outline-offset-2 has-[:focus-visible]:outline-accent-ink">
...
function RFBlock({ id, data, selected }: NodeProps<RFBlockNode>) {
  ...
  // Handles move between the sides and the top/bottom edges with the layout.
  useEffect(() => updateNodeInternals(id), [cascade.vertical, id, updateNodeInternals]);
  const frame = selected ? 'border-marker-ink' : inChain ? 'border-line-strong' : 'border-dashed border-line-strong opacity-70';
  return (
    <div className={`w-[204px] rounded-[4px] border bg-surface text-ink transition-[border-color,opacity] duration-200 ${frame}`}>
      ...
      <div className="flex items-center gap-2.5 border-b border-line py-2 pl-3 pr-1.5">
        <span
          ...
          aria-hidden="true"
        />
        <span className="font-mono text-[10.5px] text-ink-3 [font-stretch:87.5%]">{inChain ? String(index + 1).padStart(2, '0') : '—'}</span>
        ...
        <button
          type="button"
          className="nodrag grid size-7 place-items-center rounded-[3px] font-mono text-[14px] text-ink-3 transition-colors hover:bg-surface-2 hover:text-ink"
  ...
  return (
    <div className="flex flex-col border-b border-line lg:border-b-0 lg:border-r">
      <div className="px-5 pb-4 pt-5">
        <h4 className="kicker text-[10.5px]">Parts</h4>
      ...
      <ul className="grid grid-cols-1 gap-px border-y border-line bg-line sm:grid-cols-2 lg:grid-cols-1">
        {PART_ORDER.map((kind) => {
          ...
          return (
            <li key={kind} className="bg-bg">
              <button type="button" onClick={() => onAdd(kind)} className="group flex w-full items-center gap-3 px-5 py-3 text-left transition-colors hover:bg-surface-2 focus-visible:bg-surface-2">
                ...
                <span className="font-mono text-[16px] leading-none text-ink-3 transition-colors group-hover:text-ink" aria-hidden="true">
                  +
                </span>
              </button>
            </li>
      ...
      <div className="mt-auto grid grid-cols-2 gap-2 p-5">
        <button type="button" onClick={onReset} className="btn-icon h-10 w-full whitespace-nowrap px-2 font-mono text-[10px] [font-stretch:87.5%]">
          Reset example
        </button>
        <button type="button" onClick={onClear} className="btn-icon h-10 w-full whitespace-nowrap px-2 font-mono text-[10px] [font-stretch:87.5%]">
          Clear canvas
        </button>
      </div>
    </div>
      ...
      <div className="grid grid-cols-2 gap-px bg-line md:grid-cols-4">
        {cells.map((cell) => (
          <div key={cell.label} className="bg-surface p-5">
            <Readout label={cell.label} value={cell.value} unit={cell.unit} large />
          </div>
        ))}
      </div>
      {frequencyLabel && <p className="border-t border-line bg-surface px-5 py-2.5 font-mono text-[10.5px] text-ink-3 [font-stretch:87.5%]">Summary values {frequencyLabel}, the middle sample of the band the loaded files share</p>}
    </div>
  );
}
...
function SweptChart({ data }: { data: SweptCascadeResult[] }) {
  return (
    <figure className="panel p-5">
      <figcaption className="kicker mb-4 text-[10.5px]">Swept cascade across the common S-parameter band</figcaption>
      <div className="h-60">
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={data} margin={{ top: 4, right: 28, left: -6, bottom: 4 }}>
            ...
            <Tooltip
              contentStyle={{ background: 'var(--surface)', border: '1px solid var(--line-strong)', borderRadius: 4, fontFamily: 'var(--font-martian)', fontSize: 11, color: 'var(--ink)' }}
              labelStyle={{ color: 'var(--ink-3)' }}
              labelFormatter={(v) => `${(Number(v) / 1e9).toFixed(3)} GHz`}
              formatter={(v, name) => [figure(Number(v)), name]}
            />
            <Legend wrapperStyle={{ fontSize: 11, fontFamily: 'var(--font-martian)', color: 'var(--ink-2)' }} />
      ...
      <div className="flex flex-col gap-6">
        <div className="grid overflow-hidden rounded-[4px] border border-line lg:grid-cols-[248px_minmax(0,1fr)]">
          ...
          <div ref={canvasRef} className="relative h-[440px] bg-bg-raised sm:h-[480px]" style={vertical ? { height: Math.max(360, spanY + NODE_HEIGHT + 68) } : undefined}>
            ...
            <p className="kicker pointer-events-none absolute right-4 top-3.5 text-[10px]">Signal flows {vertical ? 'top to bottom' : 'left to right'}</p>
            {!nodes.length && (
        ...
        {warnings.length > 0 && (
          <ul className="flex flex-col gap-2 border-l-2 border-marker-ink pl-4 text-[13px] leading-relaxed text-ink-2">
            {warnings.map((w) => (
              <li key={w}>{w}</li>
            ))}
          </ul>
