import { vswrMetrics } from '@/lib/rfCalculators';

// Computed from the single-interface identities rather than typed in, so every column agrees.
export const vswrTable = [1.0, 1.1, 1.2, 1.3, 1.4, 1.5, 2.0, 3.0].map((vswr) => {
  const m = vswrMetrics({ kind: 'vswr', value: vswr });
  return {
    vswr: vswr.toFixed(2),
    rl: Number.isFinite(m.returnLossDb) ? m.returnLossDb.toFixed(1) : '∞',
    transLoss: m.mismatchLossDb.toFixed(3),
    reflCoeff: m.gamma.toFixed(3),
    transPower: m.transmittedPercent.toFixed(2),
    reflPower: m.reflectedPercent.toFixed(2),
  };
});
...
export const dielectricsTable = [
  ...
  { name: "GaAs (semi-insulating)", er: "12.9", tand: "0.0006", thickness: "Various" },
];
...
export const waveguideTable = [
  ...
  { gb: "BJ500", wr: "WR19", freq: "39.2~59.6", a: "4.775", b: "2.388" },
  { gb: "BJ620", wr: "WR15", freq: "49.8~75.8", a: "3.759", b: "1.880" },
